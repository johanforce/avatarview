...
internal class AvatarView : AppCompatImageView {
    companion object {
    ...
    private val wordRegex = Regex("([a-zа-яA-Z]+)", RegexOption.IGNORE_CASE)
    ...
    constructor(context: Context, attrs: AttributeSet, defStyle: Int = 0) : super(context, attrs, defStyle) {
        ...
        placeholder = additionalAttrs.getDrawable(R.styleable.AvatarView_placeholder)
    ...
    private fun initView() {
        if (rawText.isEmpty() || wordRegex.findAll(rawText).count() == 0) {
            ...
            val backgroundColor = colors[abs(rawText.hashCode() % colors.size)]
    ...
    private fun getDefaultPlaceholder() = ContextCompat.getDrawable(context, R.drawable.ic_avatar_default)
